...
route.post("/:id", auth, async (req, res) => {
    ...
    try {
        await req.app.get('client').channels.cache.find(c => c.id === server.mod_log_id).send(`<:bid_add:745106320730095635> **|**<@${req.user.id}> ${resubmit ? "re" : ""}enviou o bot <@${req.params.id}> para a verificação\n<@&${server.role_ids.bot_verifier}>`);
