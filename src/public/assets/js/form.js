...
function submit() {
    ...
    let form_items = ["botid", "prefix", "background", "redirect", "description", "invite", "support", "website", "github", "tags", "owner-ids", "note", "files", "reason", "topic", "webhook", "auths"]
    let data = {}
...
let method = "PATCH";
    fetch(`/api/report/826507789312327781`, {
        method,
        headers: {
            'Content-Type': 'application/json'
        },
   ...
   let method = "PATCH";
    fetch(`/api/cpass/${data.id}`, {
        ...
        if (!body.success) {
           ...
           location.href = `/bots/pass/${data.id}`
        }
    })
}
...
function staff() {
    ...
    let form_items = ["role", "staffid", "staffage","stafflist", "staffref", 'staffexp', 'staffquiz', 'staffquiz2', 'staffquiz3', 'staffadmin', 'staffsuper']
    let data = {}
    for (let form_item of form_items) {
        data[form_item] = $(`#${form_item}`).val()
    }
    ...
    if (location.href.includes("/bots/edit")) {
        ...
        $('#auth').click(() => {
            ...
            .then(data => {
                if (data.success) {
                    Swal.fire({
                        title: 'Seu token de autorização é',
                        icon: 'info',
                        html:
                    ...
                    Swal.fire({
                        ...
                        icon: 'error',
                        html:
                        `Houve um erro com o seu token de autorização.`,
                        showCloseButton: true,
                        focusConfirm: false,
                        confirmButtonText: 'Close',
                        confirmButtonAriaLabel: 'close',
        ...
        $('#reset').click(() => {
            ...
            .then(data => {
                ...
                if (data.success) {
                    Swal.fire({
                        title: 'Seu novo token de autorização é',
                        icon: 'info',
                        html:
                        `Seu token de autorização novo é <code>${data.auth}</code>`,
                        showCloseButton: true,
                        focusConfirm: false,
